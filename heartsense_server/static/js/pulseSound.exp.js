...
// const SCALE_MAP = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9, 9.5, 10, 10.5, 11, 11.5, 12.5]; // half-chromatic scale
const SCALE_MAP = [0, 2, 4, 7, 9]; // pentatonic scale
...
class PulseSound {
    ...
    movePitch(pitch, interval) {
        // let noteOnScale = interval % SCALE_MAP.length;
        // let octave = Math.floor(interval / SCALE_MAP.length);
        
        // if (noteOnScale < 0) {
        //     noteOnScale += SCALE_MAP.length;
        //     octave -= 1;
        // }

        // return pitch * Math.pow(2, octave + SCALE_MAP[noteOnScale] / 12);
